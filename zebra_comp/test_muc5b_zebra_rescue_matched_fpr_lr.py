...
def threshold_matching_empirical_fpr(
    ...
    """
    Choose a ZeBRA-only threshold on TRAINING negatives to match
    an empirically observed training FPR as closely as possible.

    This is used to construct the fair matched-FPR ZeBRA comparator.
    """
    negative_scores = np.asarray(
        negative_scores,
        dtype=float,
    ...
    # Candidate thresholds are unique observed scores.
    unique_scores = np.unique(
        negative_scores
    )

    # Evaluate upper-tail FPR for each candidate.
    fprs = np.array(
        [
            np.mean(
                negative_scores >= t
...
for repeat in range(
    ...
    for main_fpr in MAIN_FPRS:
        ...
        for lower_fpr in LOWER_FPRS:
            ...
            # ====================================================
            # Leakage-free matched-FPR ZeBRA-only comparator
            #
            # 1. Apply the same rescue rule to TRAINING patients.
            # 2. Measure its achieved FPR among TRAINING negatives.
            # 3. Find a ZeBRA-only threshold on TRAINING negatives
            #    that matches that FPR as closely as possible.
            # 4. Apply that threshold unchanged to the outer TEST set.
            # ====================================================
            ...
            rescue_selected_train = (
                rescue_band_train
                &
                (c_train == 1)
            )

            rescued_train_positive = (
                baseline_train_positive
                |
                rescue_selected_train
            )

            rescued_train_metrics = confusion_metrics(
                y_train,
                rescued_train_positive,
            )

            target_matched_fpr_train = (
                rescued_train_metrics[
                    "FPR"
                ]
            )

            matched_threshold, matched_train_fpr = (
            ...
            matched_zebra_positive = (
                z_test >= matched_threshold
            )

            matched_zebra = confusion_metrics(
                y_test,
                matched_zebra_positive,
            )
            ...
            REPEAT_ROWS.append({
                ...
                "baseline_LR_positive":
                    baseline[
                        "LR_positive"
                    ],

                "baseline_LR_negative":
                    baseline[
                        "LR_negative"
                    ],

                "baseline_diagnostic_odds_ratio":
                    baseline[
                        "diagnostic_odds_ratio"
                    ],
                ...
                "rescued_diagnostic_odds_ratio":
                    rescued[
                        "diagnostic_odds_ratio"
                    ],

                "matched_train_target_FPR":
                    target_matched_fpr_train,

                "matched_train_achieved_FPR":
                    matched_train_fpr,

                "matched_zebra_threshold":
                    matched_threshold,

                "matched_zebra_sensitivity":
                    matched_zebra[
                        "sensitivity"
                    ],
                ...
                "matched_zebra_LR_positive":
                    matched_zebra[
                        "LR_positive"
                    ],

                "matched_zebra_LR_negative":
                    matched_zebra[
                        "LR_negative"
                    ],

                "matched_zebra_diagnostic_odds_ratio":
                    matched_zebra[
                        "diagnostic_odds_ratio"
                    ],

                "rescue_minus_matched_sensitivity":
                    (
                        rescued[
                            "sensitivity"
                        ]
                ...
                "rescue_minus_matched_LR_positive":
                    (
                        rescued[
                            "LR_positive"
                        ]
                ...
                "rescue_minus_matched_LR_negative":
                    (
                        rescued[
                            "LR_negative"
                        ]
...
display_cols = [
    ...
    "baseline_FPR_mean",
    "rescued_FPR_mean",
    "matched_zebra_FPR_mean",
    "delta_FPR_mean",

    "matched_zebra_sensitivity_mean",
    "rescue_minus_matched_sensitivity_mean",
    "rescue_minus_matched_sensitivity_q025",
    "rescue_minus_matched_sensitivity_q975",

    "baseline_LR_positive_mean",
    "rescued_LR_positive_mean",
    "matched_zebra_LR_positive_mean",

    "baseline_LR_negative_mean",
    "rescued_LR_negative_mean",
    "matched_zebra_LR_negative_mean",
...
# ============================================================
# Dedicated matched-FPR comparison table
# ============================================================
...
MATCHED_COLS = [
    ...
    "rescued_diagnostic_odds_ratio_mean",
    "matched_zebra_diagnostic_odds_ratio_mean",

    "fraction_baseline_FN_rescued_mean",
    "FP_per_rescued_TP_mean",
]

MATCHED_COMPARISON = SUMMARY[
    [
        c
...
MATCHED_COMPARISON.to_csv(
    OUTDIR
    / "MATCHED_FPR_RESCUE_VS_ZEBRA.csv",
    index=False,
)

print()
print("=" * 96)
print("MATCHED-FPR: MUC5B RESCUE VS ZeBRA ALONE")
print("=" * 96)

print(
    MATCHED_COMPARISON.to_string(
        index=False
    )
)
...
# ============================================================
# Plot: sensitivity of rescue vs matched-FPR ZeBRA
# ============================================================
...
ax.bar(
    ...
    width=width,
    label="ZeBRA alone at matched FPR",
)
...
ax.set_title(
    "Matched-FPR comparison: MUC5B rescue vs ZeBRA alone"
)
...
fig.savefig(
    OUTDIR
    / "MATCHED_FPR_sensitivity_rescue_vs_zebra.png",
    dpi=300,
    bbox_inches="tight",
)
...
# ============================================================
# Plot: LR+ at matched operating points
# ============================================================
...
ax.set_title(
    "Matched-FPR LR+: MUC5B rescue vs ZeBRA alone"
)
...
fig.savefig(
    OUTDIR
    / "MATCHED_FPR_LRplus_rescue_vs_zebra.png",
    dpi=300,
    bbox_inches="tight",
)
...
fig.savefig(
    OUTDIR
    / "MATCHED_FPR_LRminus_rescue_vs_zebra.png",
    dpi=300,
    bbox_inches="tight",
)
...
print(
    "  0. It beats ZeBRA alone at the SAME FPR; this is the decisive comparison."
)
print(
