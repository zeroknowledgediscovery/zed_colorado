...
\subsection{Boundary-targeted \textit{MUC5B} use: held-out gain at matched false-positive rate}
The two-threshold rescue rule is evaluated leakage-free: upper and lower ZeBRA thresholds are derived from training negatives, the \textit{MUC5B} rescue is applied only to untouched outer-test subjects, and a ZeBRA-only threshold is separately selected on training data to match the rescue rule's training-set FPR before test evaluation. Across all six evaluated threshold pairs, the \emph{mean} held-out sensitivity gain over this fair matched-ZeBRA comparator is positive. For the 0.5\%/10\% rule, mean sensitivity increases by 0.0446 while mean operational FPR differs from matched ZeBRA by only $-2.53\times10^{-4}$.

We additionally resolved the stricter adjudicated-negative comparison that was previously a limitation. Each original outer split was reconstructed from its saved split seed, and the already training-derived matched-ZeBRA threshold was applied unchanged to the explicitly adjudicated-negative subjects in the untouched test set; no threshold was fitted within that subset. For the 0.5\%/10\% rule, mean adjudicated-negative FPR is 0.1292 for the original stringent ZeBRA rule, 0.1958 for rescue, and 0.2144 for the fair matched-ZeBRA comparator. Thus rescue increases FPR relative to the original more-stringent baseline, but is 0.0185 lower than matched ZeBRA on average. Rescue has lower mean adjudicated-negative FPR than matched ZeBRA in five of the six evaluated rules; the remaining 1\%/2\% rule is essentially equal ($+0.00065$). For the 0.5\%/10\% rule, rescue has lower adjudicated-negative FPR than matched ZeBRA in 81\% of outer splits. Because each split contains only about 216 explicitly adjudicated negatives on average, the split-wise 2.5th--97.5th percentile range for the rescue-minus-matched difference remains broad ($-0.0561$ to $+0.0194$ for the 0.5\%/10\% rule). We therefore interpret this analysis as resolving the direction of the earlier diagnostic concern rather than as definitive evidence of a lower adjudicated-negative FPR.

\begin{table*}[!t]
\caption{Boundary-targeted \textit{MUC5B} rescue compared with a fair ZeBRA-only rule matched on training-set operational FPR. Sensitivity and FPR differences are rescue minus matched ZeBRA. The adjudicated-negative comparison uses the same untouched outer-test subjects and the already training-derived matched threshold. Values are means across 100 repeated outer splits; the final column gives the fraction of splits in which rescue produced a lower adjudicated-negative FPR than matched ZeBRA.}
\label{tab:boundary-selected}\centering\scriptsize
\begin{tabular}{lrrrr}
\toprule
Upper/lower target FPR & $\Delta$ sensitivity (pp) & $\Delta$ operational FPR (pp) & $\Delta$ adjudicated FPR (pp) & Rescue lower (\%)\\
\midrule
0.5\% / 2\%  & +1.81 & +0.0012 & -0.37 & 51\\
0.5\% / 5\%  & +3.66 & -0.0040 & -0.60 & 61\\
0.5\% / 10\% & +4.46 & -0.0253 & -1.85 & 81\\
1\% / 2\%    & +0.96 & +0.0007 & +0.06 & 31\\
1\% / 5\%    & +2.25 & -0.0184 & -0.46 & 55\\
1\% / 10\%   & +3.35 & -0.0178 & -1.92 & 78\\
\bottomrule
\end{tabular}
\end{table*}
