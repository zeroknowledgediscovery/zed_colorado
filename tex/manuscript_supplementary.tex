...
The larger sensitivity of the most flexible estimators in very sparse tails is visible in Fig.~\ref{fig:lr-robustness}. This is why the manuscript emphasizes stable 95\% and 99\% residual scales and treats the 99.5\% estimate more cautiously. The high-ZeBRA tail remains an observed finite-range phenomenon and does not prove A1-U or A2.
...
The explicitly adjudicated-negative diagnostic applies the already learned matched thresholds unchanged to the adjudicated-negative subset of each test split. Because this subset contains only about 216 subjects per split on average, its repeated-split distribution is treated as a control diagnostic rather than definitive evidence of superiority.
...
At stringent requested false-positive rates, the clinical-history score remains substantially stronger than the genomic-only learner. Mean held-out LR$+$ at requested FPR 0.5\% is approximately 57.9 for ZeBRA, 4.28 for the genomic-only model, and 39.1 for the nonlinear combined model. At requested FPR 1\%, the corresponding values are 29.8, 3.62, and 26.7. Figure~\ref{fig:decision-consequences} summarizes both the rescue results and these stringent operating-point comparisons.
...
A descriptive ROC-hull analysis was used only to characterize geometric complementarity among observed operating points. The empirical/interpolated AUCs are 0.8142 for ZeBRA and 0.8095 for the hybrid decision rule. The convex hulls of the individual curves have AUCs 0.8219 and 0.8182, respectively; the pointwise maximum has AUC 0.8196; and the convex hull of the concatenated ZeBRA and hybrid ROC points has AUC 0.8260. These hull quantities are envelopes over evaluated operating points, not held-out performance estimates from a separately trained classifier. They are retained as a supplementary geometric diagnostic and are not used as primary evidence for the theorem.
